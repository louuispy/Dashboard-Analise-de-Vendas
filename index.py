...
for i, j in zip(raw_dataframe['Mês'].unique(), df['Mês'].unique()):
...
def graph5(month, toggle):
    ...
    fig5.update_layout(main_config, height=230, template=template)
    fig6.update_layout(main_config, height=230, template=template)
...
def graph7(toggle):
    ...
    fig7.add_trace(go.Scatter(y=df_7_group["Valor Pago"], x=df_7_group["Mês"],
